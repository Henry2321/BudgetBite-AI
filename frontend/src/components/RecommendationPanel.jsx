    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/10 p-6 backdrop-blur sm:p-8">
      ...
      {loading ? (
        <div className="mt-6 flex items-center gap-3 rounded-[1.5rem] bg-white/10 p-5 text-stone-100">
          <LoadingSpinner />
          <span>
            {locationLoading ? copy.gettingLocationShort : copy.findingNearby}
          </span>
        </div>
      ) : null}
          ...
          {recommendations.map((item, index) => (
            ...
            >
              ...
              <p className="mt-2 text-sm font-medium text-stone-500">
                {item.restaurant || copy.nearbyRestaurant} | {item.category}
              </p>
              ...
              {item.maps_url ? (
                <a
                  ...
                  className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-stone-900 px-4 py-2.5 text-sm font-bold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-stone-700 hover:shadow-md"
                >
                  {copy.mapsButton}
                </a>
              ) : null}
